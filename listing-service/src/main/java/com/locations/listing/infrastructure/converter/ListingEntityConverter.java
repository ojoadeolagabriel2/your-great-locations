...
public class ListingEntityConverter {
    ...
    public Listing getListingModel(ListingEntity entity) {
        return Listing
                ...
                .listingOwner(ListingOwner
                        ...
                        .listingPhoneNumbers(Arrays.stream(entity.getListingOwner().getListingPhoneNumbers().split(",")).collect(toList()))
                        ...
                        .listingOwnerBusinessDescription(entity.getListingOwner().getListingOwnerBusinessDescription())
    ...
    public ListingEntity getListingEntity(ListingEntity entity, Listing listing) {
        ...
        entity.setListingAddress(ListingAddressEntity
                .builder()
        ...
        entity.setListingKeyFeatures(listing.getKeyFeatures()
                .stream()
                .map(item -> ListingKeyFeatureEntity.builder().featureDescription(item).build()).collect(toSet()));

        entity.setListingImageEntities(listing.getImages()
                .stream()
                .map(item -> ListingImageEntity.builder().code(item).build()).collect(toSet()));

        entity.setListingImageThumbnailEntities(listing.getImageThumbNails()
                .stream()
                .map(item -> ListingImageThumbnailEntity.builder().code(item).build()).collect(toSet()));

        entity.setListingOwner(ListingOwnerEntity
                .builder()
